...
class SentimentIndex(Data):
    def __init__(self, dict_data: dict, EMA_window: int = 100):
        ...
        self.detrended_stocks = ReturnsAndPrices(self.dict_data).detrended_prices('all_stocks')
...
class Likelihood(Data):
    ...
    def MLE(self, distribution: str, drop_extreme: float = None, start: int = None, end: int = None) -> tuple:
        ...
        options = {'maxiter': 100000, 'ftol': 1e-10, 'gtol': 1e-10, 'xtol': 1e-10, 'disp': False}
...
class EarlyWarningIndicator(Data):
    @property
    def estimation(self, drop_extreme: float = None, window: int = 750, jump: int = 5) -> list:
        '''
        Compute the MLE
        Args:
            drop_extreme (float) : Drop sentiment index values above this threshold (optional)
        ...
        for index, date in zip(range(window, len_sentiment_index + 1, jump), self.dates[sentiment_index_obj.L:][window::5]):
            ...
            params = likelihood.MLE('beta', drop_extreme, start, end)
            params_list.append(params)
            dates_list.append(date)

        b = datetime.now()

        return [e1 - e2 for (e1, e2) in params_list]
...
class Graph(Data):
    ...
    def __init__(self, dict_data: dict):
        # Sentiment index
        super().__init__(dict_data)
        self.prices = ReturnsAndPrices(self.dict_data)
    ...
    def plot_EWI(self):
        ...
        data =  pd.concat([pd.DataFrame(self.dates, columns=['Date']),
                          pd.DataFrame(self.EWI.estimation, columns=['EWI'])], axis=1)
...
#         self.W = 2 / (self.L + 1)
#
#         self.detrended_prices = Return(self.dict_data).detrended_prices('index')
#
#     def _EMA_init(self) -> dict:
#         '''
#         Initialize the EMA, the first value is the SMA
...
#             # ema_data.iloc[self.L] = np.mean(self.stocks[stock].iloc[:self.L, 1])'''
#
#             ema_data = np.full(len(self.detrended_prices('all_stocks')[stock]), np.nan)
#             ema_data[self.L] = np.mean(self.detrended_prices('all_stocks')[stock][:self.L])
#             self.EMA_dict[stock] = ema_data
#
#         return self.EMA_dict
#
...
#             for row in range(self.L + 1, len(self.stocks[stock])):
#                 # Exponential Moving Average
#                 ema = self.W * self.detrended_prices('all_stocks')[stock][row] + (1 - self.W) * self.EMA_dict[stock][row - 1]
#                 self.EMA_dict[stock][row] = ema
#
#             # Pessimistic state
#             pessimistic_state = self.detrended_prices('all_stocks')[stock] < self.EMA_dict[stock]
#             self.pessimistic_state_dict[stock] = pessimistic_state
#
#         return self.pessimistic_state_dict
#
